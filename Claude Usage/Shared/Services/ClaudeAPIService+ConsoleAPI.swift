...
extension ClaudeAPIService {
    ...
    /// Note: The Console API may not expose this endpoint; returns empty on 404.
    func fetchAPIKeys(organizationId: String, apiSessionKey: String) async throws -> [String: String] {
        ...
        switch httpResponse.statusCode {
        case 200:
            // Try { data: [...] } wrapper first, then plain array
        ...
        case 401, 403:
            throw APIError.unauthorized
        case 404:
            // Endpoint not available on this Console API version — silently return empty
            return [:]
        default:
            throw APIError.serverError(statusCode: httpResponse.statusCode)
        }
    ...
    func fetchAPIUsageData(organizationId: String, apiSessionKey: String) async throws -> APIUsage {
        ...
        do {
            ...
            // Per-key tracking: keyId -> (total, modelCosts)
            var perKeyTotal: [String: Double] = [:]
            var perKeyModels: [String: [String: Double]] = [:]

            // Helper to process entries into both aggregate and per-key maps
            func processEntry(_ entry: UsageCostEntry, modelName: String) {
            ...
            if let costs = costResponse.costs {
                for (dateKey, entries) in costs {
                    for entry in entries {
                        ...
                        processEntry(entry, modelName: cleanName)
                        dailyTotals[dateKey, default: 0] += entry.safeTotal
                    }
                }
            }
            if let webSearchCosts = costResponse.webSearchCosts {
                for (dateKey, entries) in webSearchCosts {
                    for entry in entries {
                        processEntry(entry, modelName: "Web Search")
                        dailyTotals[dateKey, default: 0] += entry.safeTotal
                    }
                }
            }
            if let codeExecutionCosts = costResponse.codeExecutionCosts {
                for (dateKey, entries) in codeExecutionCosts {
                    for entry in entries {
                        processEntry(entry, modelName: "Code Execution")
                        dailyTotals[dateKey, default: 0] += entry.safeTotal
                    }
                }
            }
